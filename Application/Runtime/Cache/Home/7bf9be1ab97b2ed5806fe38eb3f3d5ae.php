	...
	<div class="main">
		...
		<div class="main_right">
			<div class="common_head">
				...
				<form action="/hizhu/Jobowner/jobownerlist.html" method="get">
			...
			<div class="common_main">
				<h2>职业房东列表<a style="margin-left:20px;" class="btn_a" href="/hizhu/Jobowner/updatejobowner.html?no=6&leftno=111">新增职业房东</a><a style="margin-left:150px;" href="/hizhu/Jobowner/downloadExcel?<?php echo $_SERVER["QUERY_STRING"];?>" class="btn_a">下载</a>
					...
					<table>
						...
						<tbody>
						  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td>
									<?php switch($vo["status"]): case "0": ?><a href="/hizhu/Jobowner/jobownerfollow?no=6&leftno=111&customer_id=<?php echo ($vo["customer_id"]); ?>" target="_blank">待跟进</a><?php break;?>
										<?php case "3": ?><a href="/hizhu/Jobowner/jobownerfollow?no=6&leftno=111&customer_id=<?php echo ($vo["customer_id"]); ?>" target="_blank">跟进中</a><?php break;?>
										<?php case "5": ?><a href="/hizhu/Jobowner/jobownerfollow?no=6&leftno=111&customer_id=<?php echo ($vo["customer_id"]); ?>" target="_blank">不合作</a><?php break; endswitch;?>
								</td>
								...
								<td><?php if(($vo["is_black"] == 1)): ?>是<?php else: ?><a href="#" onclick="pull_blackone('<?php echo ($vo["customer_id"]); ?>','<?php echo ($vo["mobile"]); ?>',this)">否</a><?php endif; ?></td>
								<td><?php echo ($vo['update_man']); ?></td>
								<td><?php if(($vo["signed"] == 1)): ?>是<?php else: ?>否<?php endif; ?></td>
								<td><?php echo ($vo['margin']); ?></td>
								<?php if(($vo["status"] == 0 or $vo["status"] == 3)): ?><td></td>
									<td></td>
									<td></td>
									<td></td>
								<?php else: ?>
									<td><a target="_blank" href='/hizhu/Jobowner/updatejobowner.html?no=6&leftno=111&cuid=<?php echo ($vo["customer_id"]); ?>'>修改信息</a></td>
									<td><a target="_blank" href="/hizhu/Commission/commissionlistfd.html?no=6&leftno=111&mobile=<?php echo ($vo['mobile']); ?>">修改佣金</a></td>
									<td><a target="_blank" href="/hizhu/Commission/commissionMonthly.html?no=6&leftno=111&mobile=<?php echo ($vo['mobile']); ?>&customer_id=<?php echo ($vo['customer_id']); ?>">修改付费</a></td>
									<td><a href="#" onclick="flushRoom('<?php echo ($vo["customer_id"]); ?>',this)">刷新</a></td><?php endif; ?>
...
function flushRoom(customer_id,obj){
	if(confirm('确认刷新该房东下面的所有房源？')){
		...
		$.get('/hizhu/Jobowner/reflushRoomForOwner?cuid='+customer_id,function(data){
			alert(data.message);
		},'json');
	}
}
...
function pull_blackone(customer_id,mobile,obj){
	if(confirm('确定要将该房东商务拉黑吗？')){
		$.get('/hizhu/Jobowner/pullblack_submit?customer_id='+customer_id+'&mobile='+mobile,function(data){
			alert(data.message);
			if(data.status=="200"){
				$(obj).parent().html('是');
			}
...
function batchBlack(){
	if(confirm("你正在操作非佣拉黑，是否继续？")){
		$("#blackDiv").show();
		$.get('/hizhu/Jobowner/batchblack_count?handle=getcount',function(content){
...
$("#btn_submitblack").bind("click",function(){
	...
	$.get('/hizhu/Jobowner/batchblack_submit',function(data){
		alert(data.message);
...
function checkIsowner(){
	if(confirm("你正在操作职业房东检测，是否继续？")){
		$("#checkDiv").show();
		$.get('/hizhu/Jobowner/checkIsowner_count?handle=getcount',function(content){
...
$("#btn_submitcheck").bind("click",function(){
	...
	$.get('/hizhu/Jobowner/checkIsowner_submit',function(data){
		alert(data.message);
