...
<script type="text/javascript">
	...
	<div class="main">
		...
		<div class="main_right">
			<div class="common_head">
				...
				<form action="<?php echo U('Jobowner/jobownerVerifyList');?>" method="get">
						<input type="hidden" name="no" value="6">
						<input type="hidden" name="leftno" value="176">
			...
			<div class="common_main">
				<h2>列表展示<a href="/hizhu/Jobowner/downloadJobownerExcel.html?<?php echo $_SERVER["QUERY_STRING"];?>" class="btn_a" style="min-width: 100px">下载</a></h2>
				<div class="table">
					<table>
						<thead>
							<tr>
						...
						<tbody>
						  	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td>
									<?php if($vo['house_type'] == '1'): ?>集中式
									<?php elseif($vo['house_type'] == '2'): ?>
									 分散式
									<?php elseif($vo['house_type'] == '1,2'): ?>
									 集中式-分散式<?php endif; ?>
								</td>
								<td><?php echo ($vo['house_num']); ?></td>
								<td><?php echo (date("Y-m-d H:i:s",$vo['create_time'])); ?></td>
								<td>
									<?php if($vo['refuse_status'] == '0'): ?><a href="javascript:;" onclick="IsPass('<?php echo ($vo[id]); ?>','<?php echo ($vo[customer_id]); ?>','<?php echo ($vo[mobile]); ?>',1);">通过&nbsp;&nbsp;</a>
									<a href="/hizhu/Jobowner/refuseReason.html?no=6&leftno=176&origin=1&id=<?php echo ($vo["id"]); ?>">&nbsp;&nbsp;拒绝</a>
									...
									<a href="javascript:;" onclick="IsPass('<?php echo ($vo[id]); ?>','<?php echo ($vo[customer_id]); ?>','<?php echo ($vo[mobile]); ?>',1);">通过</a>
									--已拒绝<?php endif; ?>
								</td>
	...
	function IsPass(id,customer_id,mobile,refuse_status){
		$("#infoAdd").show();
		...
		$("#btn_submit").click(function(){
			...
			$.ajax({
		            type:"post",
		            url: "<?php echo U('Stores/modifyCustomerCheckVerify');?>",
