	...
	<div class="main">
		...
		<div class="main_right">	
			<div class="common_main">
				...
				<a href="<?php echo U('Jobowner/ownersUpdate?no=6&leftno=204',array('id'=>$infoList['id']));?>" class="btn_a" style="margin-left:50px" target="_blank">编辑</a>
				</div>
				<table  class="table_one table_two" style="margin:10px 0 30px 50px;width:70%;text-align:center;">
					...
					<tbody>
						<tr>
							...
							<td>
								<?php switch($infoList["city_code"]): case "001009001": ?>上海<?php break; case "001001": ?>北京<?php break; case "001011001": ?>杭州<?php break; case "001010001": ?>南京<?php break; case "001019002": ?>深圳<?php break; endswitch;?>
							</td>
				...
				<a href="<?php echo U('Jobowner/ownerPortAdd?no=6&leftno=204',array('id'=>$infoList['id']));?>" class="btn_a" style="margin-left:50px" target="_blank" id="portAdd">新增</a>
				...
				<table  class="table_one table_two" style="margin:10px 0 30px 50px;width:70%;text-align:center;">
					...
					<?php if($portList != null): ?><tr>
							...
							<td><?php echo round(($portList['service_end']-$portList['service_start'])/86400) ?></td>
							<td><?php if(($portList["service_end"] > 0)): echo (date("Y-m-d",$portList["service_end"])); endif; ?></td>
							<td><?php echo ($portList["links_num"]); ?></td>
							<td>
								<?php switch($portList["city_code"]): case "001009001": ?>上海<?php break; case "001001": ?>北京<?php break; case "001010001": ?>南京<?php break; case "001011001": ?>杭州<?php break; case "001019002": ?>深圳<?php break; endswitch;?>
							</td>
							...
							<td>
								...
								<a href="<?php echo U('Jobowner/ownerPortDelay',array('id'=>$portList['id'],'customer_id'=>$infoList['id']));?>" class="btn_a" target="_blank" id="portDelay">延期&nbsp;</a>
							</td>
				...
				<?php if($monthlyList["is_open"] == 0): ?><a href="<?php echo U('Jobowner/ownerMonthlyAdd?no=6&leftno=204',array('id'=>$infoList['id']));?>" class="btn_a" style="margin-left:80px" target="_blank">新增</a><?php endif; ?>
				</div>
				<table  class="table_one table_two" style="margin:10px 0 30px 50px;width:70%;text-align:center;">
					<thead>
						<tr>
				...
				<?php if($commissionList["is_open"] == 0): ?><a href="<?php echo U('Jobowner/ownerCommissionAdd?no=6&leftno=204',array('id'=>$infoList['id']));?>" class="btn_a" style="margin-left:80px" target="_blank" id="isOpen">新增</a><?php endif; ?>
				</div>
				<table  class="table_one table_two" style="margin:10px 0 30px 50px;width:70%;text-align:center;">
					<thead>
						<tr>
...
$(document).ready(function(e) {
	...
	if(portEndTime*1000 > timestamp+60000) {
		$("#portAdd").hide();
	} else {
		$("#portDelay").hide();
	}
})
function portstopdown(id,cid,time){
		if(confirm('确认停用端口吗？')){
			$.get('/hizhu/Jobowner/ownerPortStop?id='+id+'&customer_id='+cid+'&service_start='+time,function(data){
				alert(data.message);
				document.location.reload();
			},"json");
		}
}
function monthlystopdown(id,cid){
		if(confirm('确认停用包月吗？')){
