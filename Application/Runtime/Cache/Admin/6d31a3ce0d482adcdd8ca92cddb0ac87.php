	...
	<h3>房源详情</h3>
		<table class="table">
			...
			<tr>
				...
				<td><?php echo ($list["beds"]); ?>张</td>
				<td><?php echo ($list["toilets"]); ?>个</td>
	...
	<h3>租户详情</h3>
		<table class="table">
			...
			<tr>
				...
				<td><?php echo ($list["price"]); ?></td>
				<td><?php if($list["status"] == 1): ?>未确认<?php else: ?>已确认<?php endif; ?></td>
				<td><?php if($list["ispay"] == 1): ?>未支付<?php else: ?>已支付<?php endif; ?></td>
				<td><?php if($list["cancel"] == 1): ?>已入住<?php else: ?>订单取消<?php endif; ?></td>
				
			
			</tr>
		</table>
	<a href="<?php echo U('Orders/index');?>">返回订单首页</a>
