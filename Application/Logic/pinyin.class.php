...
class pinyin {
	...
	public function getPinyin($str, $pix = '', $code = 'utf8') {
	...
	public function getFirstChars($str = ''){
		$chars = preg_split("//u", $str, -1, PREG_SPLIT_NO_EMPTY);
		//print_r($chars);
		$newstr='';
		foreach ($chars as $c){
			$newstr.=$this->getFirstChar($c);
		}
		return $newstr;
	}
	function str_split_unicode($str, $l = 0) {
		/* if ($l > 0) {
			$ret = array();
			$len = mb_strlen($str, $this->_outEncoding);
			for ($i = 0; $i < $len; $i += $l) {
				$ret[] = mb_substr($str, $i, $l, $this->_outEncoding);
			}
			return $ret;
		} */
	}
